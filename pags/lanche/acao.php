...
function create(){
    ...
    $sql = "INSERT INTO lanche (sabor, valor, descricao, imagem) VALUES('{$data['flavor']}',
...
function remove(){
    ...
    $stmt = $connection->prepare("DELETE FROM lanche WHERE codigo=:code");
...
function update ($code){
    ...
    $data = formToArray();
    $sql = "UPDATE lanche SET sabor='{$data['flavor']}', valor={$data['value']}, descricao='{$data['description']}',
...
function findById($code){
    $connection = Conexao::getInstance();
    $query = $connection->query("SELECT * FROM lanche WHERE codigo=$code;");
